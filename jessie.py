...
async def on_message(message):
    ...
    if lotto == 7777:
        if message.author.id == "246437474463776769":
            ...
            authid = str(authid)
            await client.send_message(message.channel, "**OH MY GOD** <@%s> **JUST ROLLED A 7777 OUT OF 10000 POSSIBLE NUMBERS%s** **THAT'S A WOPPING 0.01%s%s** **GIVE** <@%s> **AN APPLAUSE%s** :clap::clap::clap:" % (authid,"!","%","!",authid,"!!!!!"))
    for a in gays:
        if a in mess:
